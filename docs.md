...
<code class="html" x-include="/admin-console/components/views/Feedback/get.html"></code>
The source of the above example is located at: /admin-console/components/views/Feedback/get.html 

#### Basic concept
```html
<draftsman-query alias="Feedbackget" x-include="/prepared-statements/views/Feedback/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/Feedback/get.txt) to an Alpine data object (Feedbackget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'Feedback'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.Feedbackget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="Feedbackget" x-include="/form-definitions/Feedback/get.json"></div>
```
...
from the view updater indicating that view 'Feedback' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.feedback.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.feedback.get.key' refers to previously retrieved data using a query tag.
```html
...
<code class="html" x-include="/admin-console/components/commands/CreateFeedback.html"></code>

#### The basic concept
```html
<draftsman-mutation command="CreateFeedback" x-include="/prepared-statements/commands/CreateFeedback.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/CreateFeedback.txt) as a command (CreateFeedback) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="CreateFeedback" x-include="/form-definitions/CreateFeedback.json"></div>
```
...
```javascript
$store.mutation.send('CreateFeedback',{all-form-data});
```
...
<draftsman-trace command="CreateFeedback" 
                 ...
                 @trace="
                    ...
                    $dispatch('setdata', { command: 'CreateFeedback', data: {} });">
</draftsman-trace>
```
